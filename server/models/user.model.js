...
UserSchema.statics = {
   ...
   */
  get(id) {
    return this.findById(id,{password:0,createdAt:0})
      .exec()
      .then((user) => {
        if (user) {
          return user;
   ...
   */
  getWithLogin(login) {
    return this.findOne({$or:[{username:login},{mail:login}]})
      .exec()
      .then((user) => {
        if (user) {
          return user;
  ...
  checkUsernamePresent(name) {
    return this.findOne({username:name},{password:0,createdAt:0})
      .exec()
      .then((user) => {
        if (user) {
  ...
  checkEmailPresent(email) {
    return this.findOne({mail:email},{password:0,createdAt:0})
      .exec()
      .then((user) => {
        if (user) {
